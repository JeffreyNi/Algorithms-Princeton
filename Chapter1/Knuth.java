...
/**
 * The Knuth class provides a client for reading in a sequence of string
 * and shuffling them using the Knuth (or Fisher-Yates) shuffling algorithm.
 * This algorithm guarantees to rearrange the elements in uniformly random 
 * order, under the assumption that Math.random() generates independent and
 * uniformly distributed numbers between 0 and 1.
 *
...
public class Knuth {
	...
	int N = a.length;
	for(int i = 0; i < N; i++) {
	    // choose index uniformly from [i, N - 1]
    ...
    /**
     * Reads in a sequence of strings from standard input, shuffles them and
